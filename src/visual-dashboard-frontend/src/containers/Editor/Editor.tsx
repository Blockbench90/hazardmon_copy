...
class Editor extends React.Component<SchemaEditorProps, SchemaEditorState> {
    ...
    private renderTitle() {
        ...
        const {schemaName, showSchemaNameForm} = this.state;
    ...
    private onDrop(event: any) {
        if (event.dataTransfer.getData("storm-diagram-node")) {
            ...
            const node = new CustomNodeModel(data.anchor_points);
            node.extras = {...data, sensors: [], scale: 1};
    ...
    return {
        ...
        saveForm: (schemaName: string, name: string, app: any, dimensions: any) => {
            ...
            serializedApp.links.forEach((link: any) => {
                ...
                const pointsAdditional = additionalPoints.map((point: any) => ({x_coord: point.x, y_coord: point.y}))
                connector.additional_points = pointsAdditional
