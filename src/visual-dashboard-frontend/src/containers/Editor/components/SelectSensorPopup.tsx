...
class SelectSensorPopup extends React.Component<SelectSensorPopupProps, SelectSensorPopupState> {
    ...
    public render() {
        const { selectedSensorInfo, suggestionsInputValue, sensorSuggestions, app, closeSuggestionsBlock,
            toggleSensor, schemaDetails, tabDetails, onTabSensors, locations, match, tabName } = this.props;
                            ...
                            {sensorSuggestions.map((sensor: any) => {
                                ...
                                return (
                                    <div key={sensor.id} className={resultClassName} onClick={handleSensorClick}>
                                        ...
                                        {disabled && <div className="already-in-use">(Already in use at {usedOn ? usedOn : 'Current tab'} [{machineName ? machineName : 'Untitled machine'}])</div>}
                                    </div>
                                );
                            })}
                        </div>
                        {(selectedSensorInfo.sensor && selectedSensorInfo.sensor.sensor) &&
                        <button className="btn btn-danger small" onClick={handleClearSensor}>Remove {selectedSensorInfo.sensor.sensor.name}</button>
                        }
                    ...
                    <div className="select-sensor-filters">
                        <div className="filter-wrapper">
                            ...
                            <ul>
                                <li>
                                    <label>
                                        <input type="checkbox" name="excludeGroup" onChange={this.toggleFilterAll} checked={!excludeGroup.length} />
                                        <div>Select All</div>
                                    </label>
                                </li>
                                {sensorGroups.map((groupObj: any) => {
                                    return (
                                        <li key={groupObj.value}>
                                            <label>
                                                <input type="checkbox" value={groupObj.value} name="excludeGroup" onChange={this.handleFilterChange} checked={!excludeGroup.includes(groupObj.value)} />
                        ...
                        <div className="filter-wrapper">
                            ...
                            <ul>
                                <li>
                                    <label>
                                        <input type="checkbox" name="excludeDevices" onChange={this.toggleFilterAll} checked={!excludeDevices.length} />
                                        <div>Select All</div>
                                    </label>
                                </li>
                                {availableDevices.map((device: any) =>
                                    <li key={device.id}>
                                        <label>
                                            <input type="checkbox" value={device.id} name="excludeDevices" onChange={this.handleFilterChange} checked={!excludeDevices.includes(device.id.toString())} />
                                            <div>{device.name}</div>
                                        </label>
                                    </li>
                                )}
